...
import org.littlestar.tpcc.TransactionType;
...
public interface RandomHelper {
	...
	public static TransactionType randomTransaction(final float paymentRation, final float orderStatusRation, final float deliveryRation, final float stockLevelRation) {
		float chance = RandomHelper.randomFloat();
		if (paymentRation >= chance) {
			return TransactionType.Payment;
		} else if (paymentRation < chance && chance <= (paymentRation + orderStatusRation)) { // paymentRation + orderStatusRation = 0.47
			return TransactionType.OrderStatus;
		} else if ((paymentRation + orderStatusRation) < chance && chance <= (paymentRation + orderStatusRation + deliveryRation)) { // paymentRation + orderStatusRation + deliveryRation = 0.51;
			return TransactionType.Delivery;
		} else if ((paymentRation + orderStatusRation + deliveryRation) < chance && chance <= (paymentRation + orderStatusRation + deliveryRation + stockLevelRation)) { // paymentRation + orderStatusRation + deliveryRation + stockLevelRation = 0.55;
